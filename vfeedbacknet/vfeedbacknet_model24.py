...
from vfeedbacknet.vfeedbacknet_feedbackConvLSTM import FeedbackConvLSTMCell_v1
from vfeedbacknet.vfeedbacknet_feedbackConvLSTM import feedback_dynamic_rnn
...
class Model:
    ...
    def _declare_variables(self):
        ...
        with tf.variable_scope('vfeedbacknet_{}'.format(Model.model_name)):
            ...
            with tf.variable_scope('feedbackconvlstm1'):
                with tf.variable_scope('rnn'):
                    with tf.variable_scope('feedback_conv_lstm_cell_v1'):
                        ...
                        fb_channels = 256
                        m = 4*x_channels
                        input_size = [28, 28, x_channels]
                        kernel2d_size = [3, 3]
                        kernel_size = kernel2d_size + [2*x_channels+fb_channels] + [m] 
                ...
                self.feedbackConvLSTMLayers['feedbackconvlstm1'] = FeedbackConvLSTMCell_v1(input_size[:2], x_channels, [3, 3])
            ...
            with tf.variable_scope('fc'):
                ...
                weight = tf.get_variable('weights', shape=[200704, self.num_classes], dtype=tf.float32, initializer=initializer, regularizer=regularizer, trainable=trainable)
    ...
    def __call__(self, inputs, inputs_sequence_length):
        ...
        h, w, c = featurizer_outputs[0].shape[1:]
        zeros = tf.zeros(shape=[self.batch_size, h, w, c], dtype=tf.float32)
        feedback_initial_states = [ zeros for _ in featurizer_outputs ]
        ...
        fb_cell_states = list(map(lambda x : x['cell_state'], feedback_outputs11))
        ModelLogger.log('feedback_block1', inputs)

        inputs, feedback_state = self.FeedbackConvLSTM_layer1(featurizer_outputs, feedback_initial_states, inputs_sequence_length, var_list=self.main_model_variables)
        ModelLogger.log('feedbackconvlstm_block1', inputs)
    ...
    def FeedbackConvLSTM_layer1(self, inputs, feedback_states, inputs_sequence_length, var_list=None):    
        return self.FeedbackConvLSTM_layer(1, inputs, feedback_states, inputs_sequence_length, var_list=None)
        
    def FeedbackConvLSTM_layer2(self, inputs, feedback_states, inputs_sequence_length, var_list=None):    
        return self.FeedbackConvLSTM_layer(2, inputs, feedback_states, inputs_sequence_length, var_list=None)

    def FeedbackConvLSTM_layer3(self, inputs, feedback_states, inputs_sequence_length, var_list=None):    
        return self.FeedbackConvLSTM_layer(3, inputs, feedback_states, inputs_sequence_length, var_list=None)

    def FeedbackConvLSTM_layer(self, layernum, inputs, feedback_states, inputs_sequence_length, var_list=None):

        name = 'feedbackconvlstm{}'.format(layernum)
        ...
        with tf.variable_scope('vfeedbacknet_{}'.format(Model.model_name)):
            with tf.variable_scope(name):
                ...
                lstm_outputs, feedback_states, lstm_states = feedback_dynamic_rnn(
                    self.feedbackConvLSTMLayers[name],
                    tf.stack(inputs, axis=1),
                    tf.stack(feedback_states, axis=1),
                    dtype=tf.float32,
                    sequence_length=None,
                )

                lstm_outputs = tf.unstack(lstm_outputs, axis=1)
                #feedback_states = tf.unstack(feedback_states, axis=1)
                print(lstm_states)
                
                return lstm_outputs, feedback_states
...
if __name__ == '__main__':
    ...
    batch_size = 16
    video_length = 20
    x = tf.placeholder(tf.float32, [batch_size, video_length, 224, 224, 3], name='inputs')
    x_len = tf.placeholder(tf.float32, [batch_size], name='inputs_len')
    zeros = tf.placeholder(tf.float32, [video_length], name='inputs_len')
    labels = tf.placeholder(tf.float32, [batch_size], name='inputs_len')

    model = Model(sess, 27, batch_size, train_featurizer='FROM_SCRATCH', train_main_model='FROM_SCRATCH', train_fc='FROM_SCRATCH', weights_filename='/home/jemmons/data/vfeedbacknet_base_weights.npz')

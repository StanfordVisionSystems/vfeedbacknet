...
def nofeedbacknet_resnet_small(video_length, video_width, video_height, num_labels, input_placeholder, input_length, output_placeholder, zeros):
    '''
    ...
    with tf.device("/device:CPU:0"):
        ...
        w_fc = tf.Variable( tf.truncated_normal([num_filters, num_labels], stddev=0.1, ) )
    ...
    logger.log('final_output', final_outputs)

    # ACCURACY AND LOSS ########################################################
    with tf.device("/device:CPU:0"):
        softmaxes = [ tf.nn.softmax(logits=output) for output in final_outputs ]
        logger.log('softmaxes', softmaxes)
        ...
        logger.log('cross_entropies', cross_entropies)

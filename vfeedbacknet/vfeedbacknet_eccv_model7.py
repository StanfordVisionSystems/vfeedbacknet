...
from vfeedbacknet.vfeedbacknet_feedbackCell import FeedbackLSTMCell_stack2
...
class Model:
    ...
    def _declare_variables(self):
        ...
        with tf.variable_scope('vfeedbacknet_{}'.format(Model.model_name)):
            ...
            with tf.variable_scope('feedbackcell1'):
                self.feedbackLSTMCell1 = FeedbackLSTMCell_stack2([28,28,64], 4)  
            ...
            with tf.variable_scope('reshape_convs'):
                with tf.variable_scope('conv1'):
                    ...
                    kernel = tf.get_variable('kernel', shape=[7, 7, 3, 16], dtype=tf.float32, regularizer=regularizer, initializer=initializer)
                    biases = tf.get_variable('biases', shape=[16], dtype=tf.float32, regularizer=regularizer, initializer=initializer)
                ...
                with tf.variable_scope('conv2'):
                    ...
                    kernel = tf.get_variable('kernel', shape=[3, 3, 16, 64], dtype=tf.float32, regularizer=regularizer, initializer=initializer)
            ...
            with tf.variable_scope('fc1'):
                ...
                weight = tf.get_variable('weights', shape=[128, 128], dtype=tf.float32, initializer=initializer, regularizer=regularizer, trainable=trainable)
                biases = tf.get_variable('biases', shape=[128], dtype=tf.float32, initializer=initializer, regularizer=regularizer, trainable=trainable)
            ...
            with tf.variable_scope('fc2'):
                ...
                weight = tf.get_variable('weights', shape=[128, self.num_classes], dtype=tf.float32, initializer=initializer, regularizer=regularizer, trainable=trainable)
    ...
    def __call__(self, inputs, inputs_sequence_length):
        ...
        inputs = [ self.reshape_conv_layer(inp, 2, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('reshape_conv_layer2', inputs)
        
        inputs = [ self.feedbackLSTMCell1.apply_layer([inp, inp, inp, inp], var_list=self.featurizer_variables) for inp in inputs]
        fb_sequence = [ [] for _ in range(4) ]
        for inp in inputs:
            fb_sequence[0].append(inp[0])
            fb_sequence[1].append(inp[1])
            fb_sequence[2].append(inp[2])
            fb_sequence[3].append(inp[3])
        ModelLogger.log('feedbackCell1', fb_sequence[0])        

        logits = []
        for fbi in range(4):

            inputs = [ tf.nn.max_pool(inp, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID') for inp in fb_sequence[fbi] ]
            ModelLogger.log('fb{}_maxpool'.format(fbi), inputs)
            
            inputs = [ self.reshape_conv_layer(inp, 3, var_list=self.featurizer_variables) for inp in inputs ]
            ModelLogger.log('fb{}_reshape_conv_layer3'.format(fbi), inputs)

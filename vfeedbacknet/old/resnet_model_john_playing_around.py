...
This source file was obtained from: https://github.com/tensorflow/models/blob/master/official/resnet/resnet_model.py
...
def imagenet_resnet_v2_generator(block_fn, layers, num_classes,
  ...
  def model(inputs, is_training):
    ...
    inputs = conv2d_fixed_padding(
        inputs=inputs, filters=32, kernel_size=5, strides=2,
        data_format=data_format)
    inputs = tf.identity(inputs, 'initial_conv')
    ...
    inputs = block_layer(
        inputs=inputs, filters=32, block_fn=block_fn, blocks=layers[0],
        strides=2, is_training=is_training, name='block_layer1',
        data_format=data_format)
    # add convLSTM block (which will eventually be the end of a feedback block

    inputs = block_layer(
        inputs=inputs, filters=64, block_fn=block_fn, blocks=layers[1],
        strides=2, is_training=is_training, name='block_layer2',
        data_format=data_format)
    # add convLSTM block (which will eventually be the end of a feedback block

    inputs = block_layer(
        inputs=inputs, filters=128, block_fn=block_fn, blocks=layers[2],
        strides=2, is_training=is_training, name='block_layer3',
        data_format=data_format)
    # add convLSTM block (which will eventually be the end of a feedback block

    inputs = block_layer(
        inputs=inputs, filters=256, block_fn=block_fn, blocks=layers[3],
    ...
    inputs = tf.reshape(inputs,
                        [-1, 256 if block_fn is building_block else 1024])
    inputs = tf.layers.dense(inputs=inputs, units=num_classes)
    inputs = tf.identity(inputs, 'final_dense')
    return inputs

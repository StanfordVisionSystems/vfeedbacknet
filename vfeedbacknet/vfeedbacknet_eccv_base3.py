...
class Model:
    ...
    def _declare_variables(self):
        ...
        with tf.variable_scope('vfeedbacknet_{}'.format(Model.model_name)):
            ...
            with tf.variable_scope('feedbackcell11'):
                self.feedbackLSTMCell11 = FeedbackLSTMCell_RB3([112, 112, 32], Model.NFEEDBACK_ITERATIONS)

            with tf.variable_scope('feedbackcell12'):
                self.feedbackLSTMCell12 = FeedbackLSTMCell_RB3([112, 112, 32], Model.NFEEDBACK_ITERATIONS)

                
            with tf.variable_scope('feedbackcell21'):
                self.feedbackLSTMCell21 = FeedbackLSTMCell_RB3([56, 56, 64], Model.NFEEDBACK_ITERATIONS)

            with tf.variable_scope('feedbackcell22'):
                self.feedbackLSTMCell22 = FeedbackLSTMCell_RB3([56, 56, 64], Model.NFEEDBACK_ITERATIONS)

                
            with tf.variable_scope('feedbackcell31'):
                self.feedbackLSTMCell31 = FeedbackLSTMCell_RB3([28, 28, 128], Model.NFEEDBACK_ITERATIONS)

            with tf.variable_scope('feedbackcell32'):
                self.feedbackLSTMCell32 = FeedbackLSTMCell_RB3([28, 28, 128], Model.NFEEDBACK_ITERATIONS)

                
            with tf.variable_scope('feedbackcell41'):
                self.feedbackLSTMCell41 = FeedbackLSTMCell_RB3([14, 14, 256], Model.NFEEDBACK_ITERATIONS)

            with tf.variable_scope('feedbackcell42'):
                self.feedbackLSTMCell42 = FeedbackLSTMCell_RB3([14, 14, 256], Model.NFEEDBACK_ITERATIONS)
            ...
            with tf.variable_scope('fc1'):
                ...
                weight = tf.get_variable('weights', shape=[1024, 1024], dtype=tf.float32, initializer=initializer, regularizer=regularizer, trainable=trainable)
                biases = tf.get_variable('biases', shape=[1024], dtype=tf.float32, initializer=initializer, regularizer=regularizer, trainable=trainable)
    ...
    def __call__(self, inputs, inputs_sequence_length):
        ...
        ## main model ##
        inputs = [ self.feedbackLSTMCell11.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell11', inputs[0])
        
        inputs = [ self.feedbackLSTMCell12.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell12', inputs[0])
        ...
        inputs = [ self.feedbackLSTMCell21.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell21', inputs[0])
            
        inputs = [ self.feedbackLSTMCell22.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell22', inputs[0])
        ...
        inputs = [ self.feedbackLSTMCell31.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell31', inputs[0])
            
        inputs = [ self.feedbackLSTMCell32.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell32', inputs[0])
        ...
        inputs = [ self.feedbackLSTMCell41.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell41', inputs[0])
            
        inputs = [ self.feedbackLSTMCell42.apply_layer(inp, var_list=self.featurizer_variables) for inp in inputs ]
        ModelLogger.log('feedback_cell42', inputs[0])
